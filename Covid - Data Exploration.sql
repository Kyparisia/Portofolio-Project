...
-- 1b. Continent column without values:
SELECT * FROM coviddeaths
WHERE continent =''
GROUP BY location;

-- 1c. Taking out Africa, Asia, Europe, European Union, International,North America,South America,Oceania,World) We have World results in this way
SELECT * FROM coviddeaths
WHERE continent <>'';

------------------------------------------------------------------------------------------------------------

-- 2. Total Cases vs Total Deaths per day
...
-- 3. Total Cases vs Population per day
...
-- 4. Countries(Location) with Highest Infection Rate compared to Population
...
-- 5. Countries(Location) with Highest Death Count per Population
SELECT location, MAX(total_deaths) as TotalDeathCount FROM coviddeaths
WHERE continent <>''
GROUP BY location
ORDER BY TotalDeathCount DESC;

-- 6. Continents(continent) with Highest Death Count per Population
...
-- 8. Showing the RollingPeopleVaccinated every day
